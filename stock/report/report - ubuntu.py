...
class StockAnalysis:
    ...
    def stockup(self, db, cursor, tablename):
        ...
        for i in list(range(len(data_stock))):
            ...
            order_look_order = """select c.num_new from (select (@rownum:=@rownum+1) as num_new, b.* from (select * from
                                (select * from {0} order by rate_250d desc limit 500) as a
                                where rate_125d > 50 and rate_22d > 30 and rate_250d != rate_125d) as b,
                                (select @rownum:=0) as d order by rate_22d desc limit 100) as c where code = {1};"""
    ...
    def email_send(self, messages):
        ...
        # 接收邮箱
        receivers = ['8780037@qq.com', 'admin@pooper.tk']
        # 发送邮件主题
